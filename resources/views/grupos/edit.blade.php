...
@section('title', __('grupo.edit_title'))
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('grupo.edit_title') }}</h1>
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">{{ __('grupo.edit_subtitle') }}</p>
    </div>
</div>
    ...
    <form action="{{ route('grupos.update', $grupo) }}" method="POST">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div class="sm:col-span-2">
                ...
                <input type="text" name="nome" value="{{ old('nome', $grupo->nome) }}" placeholder="{{ __('grupo.name_placeholder') }}" required
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="descricao" rows="3" placeholder="{{ __('grupo.description_placeholder') }}"
                          class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('descricao', $grupo->descricao) }}</textarea>
            </div>

            <div>
                <label class="block text-xs text-gray-500 uppercase tracking-wide font-semibold mb-1.5">{{ __('grupo.color') }}</label>
                <div class="flex items-center gap-3">
                    <input type="color" name="cor" value="{{ old('cor', $grupo->cor ?? '#6366f1') }}"
                           class="w-10 h-10 rounded-lg border border-gray-200 cursor-pointer p-0.5">
                    <input type="text" name="cor" value="{{ old('cor', $grupo->cor ?? '#6366f1') }}" placeholder="{{ __('grupo.color_placeholder') }}"
