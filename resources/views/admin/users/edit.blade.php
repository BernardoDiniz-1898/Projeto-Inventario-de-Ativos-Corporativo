...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('user.edit_title') }}</h1>
    <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">{!! __('user.edit_hint', ['name' => '<strong>' . $user->name . '</strong>']) !!}</p>
</div>

<form action="{{ route('admin.users.update', $user) }}" method="POST" class="form-card bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 sm:p-8">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            <label for="password" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('user.field.new_password') }}</label>
        ...
        <div>
            <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{{ __('user.field.new_password_confirm') }}</label>
