...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">{{ __('employee.edit_title') }}</h1>
    <p class="text-gray-500 text-sm mt-1">{{ __('employee.edit_hint', ['name' => $employee->nome]) }}</p>
</div>

<form action="{{ route('employees.update', $employee) }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 lg:col-span-3">
            ...
            <textarea id="observacoes" name="observacoes" rows="3"
                      class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition @error('observacoes') border-red-300 @enderror">{{ old('observacoes', $employee->observacoes) }}</textarea>
            @error('observacoes')
                <p class="text-red-500 text-xs mt-1.5">{{ $message }}</p>
            @enderror
        </div>
