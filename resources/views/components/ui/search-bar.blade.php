...
<form method="GET" action="{{ $action }}" class="p-4 flex flex-col sm:flex-row gap-3">
    ...
    @if(!empty($filterOptions))
        <select name="{{ $filterOptions['name'] }}" class="select-chevron w-full sm:w-auto border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-gray-900 dark:text-gray-100 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
