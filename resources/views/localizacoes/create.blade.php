...
@section('title', __('localizacao.create_title'))
...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('localizacao.create_title') }}</h1>
    <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">{{ __('localizacao.create_subtitle') }}</p>
</div>

<form action="{{ route('localizacoes.store') }}" method="POST" class="form-card bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 sm:p-8">
    @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div>
            ...
            <input type="text" id="predio" name="predio" value="{{ old('predio') }}"
    ...
    <div class="mt-8 pt-6 border-t border-gray-100 dark:border-slate-700 flex items-center gap-3">
        ...
        <span class="text-xs text-gray-400 ml-auto">{{ __('localizacao.required_fields') }}</span>
    </div>
</form>
@endsection
