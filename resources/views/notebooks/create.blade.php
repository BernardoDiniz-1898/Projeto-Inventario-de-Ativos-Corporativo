...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('notebook.create_title') }}</h1>
    <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">{{ __('notebook.create_subtitle') }}</p>
</div>

<form action="{{ route('notebooks.store') }}" method="POST" class="form-card bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 p-6 sm:p-8">
    @csrf
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div>
            ...
            <select id="localizacao_id" name="localizacao_id"
                ...
                @if (old('localizacao_id'))
                    <template x-if="!filteredLocalizacoes.find(l => l.id == {{ old('localizacao_id') }})">
                        <option value="{{ old('localizacao_id') }}" selected>{{ old('localizacao_id') }}</option>
                    </template>
                @endif
            </select>
